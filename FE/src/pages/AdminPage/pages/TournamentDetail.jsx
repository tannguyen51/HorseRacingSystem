...
const raceStatusBg = (s) => ({scheduled:"rgba(37,99,235,0.1)",inprogress:"rgba(245,158,11,0.1)",finished:"rgba(16,185,129,0.1)",cancelled:"rgba(239,68,68,0.1)",awaitingresult:"rgba(139,92,246,0.1)",resultpendingapproval:"rgba(245,158,11,0.1)",resultapproved:"rgba(16,185,129,0.1)"})[s]||"rgba(100,116,139,0.1)";
const raceStatusColor = (s) => ({scheduled:"#2563eb",inprogress:"#f59e0b",finished:"#10b981",cancelled:"#ef4444",awaitingresult:"#8b5cf6",resultpendingapproval:"#f59e0b",resultapproved:"#047857"})[s]||"#64748b";
const raceStatusLabel = (s) => ({scheduled:"Sắp diễn ra",inprogress:"Đang đua",finished:"Đã kết thúc",cancelled:"Đã hủy",awaitingresult:"Chờ kết quả",resultpendingapproval:"Chờ duyệt",resultapproved:"Đã duyệt KQ"})[s]||s||"Không xác định";
  ...
  return (
    <span style={{display:"flex",alignItems:"center",gap:4}}>
      <input type="number" step="0.01" min="1" value={val} onChange={(e)=>setVal(e.target.value)}
        style={{width:60,padding:"3px 6px",borderRadius:6,border:"1px solid rgba(143,100,32,0.2)",fontSize:12}} />
      <button onClick={save} disabled={saving} style={{padding:"3px 8px",borderRadius:6,border:"none",background:"#e6a54a",color:"#fff",cursor:"pointer",fontSize:11}}>{saving?"...":"Lưu"}</button>
    </span>
  );
}
      ...
      return <div key={id} style={{borderRadius:12,border:"1px solid rgba(143,100,32,0.16)",background:"rgba(255,250,240,0.96)",overflow:"hidden"}}>
        <div role="button" tabIndex={0} onClick={toggleExpand} onKeyDown={(e)=>{if(e.key==="Enter"||e.key===" "){e.preventDefault();toggleExpand();}}} style={{padding:"12px 16px",display:"flex",justifyContent:"space-between",alignItems:"center",cursor:"pointer",flexWrap:"wrap",gap:8}}>
          ...
          <div style={{display:"flex",gap:6,alignItems:"center",flexWrap:"wrap"}}>
            {st==="scheduled"&&<button style={{padding:"6px 14px",fontSize:12,borderRadius:6,border:"none",background:"#e6a54a",color:"#fff",cursor:"pointer",fontWeight:600,opacity:det.refAssignments?.some(r=>(r.status||r.Status)==="Confirmed")?1:0.5}} disabled={!det.refAssignments?.some(r=>(r.status||r.Status)==="Confirmed")} onClick={async(e)=>{e.stopPropagation();try{await request(`/api/races/management/${id}/start`,{method:"POST"});setMessage("Đã bắt đầu!");viewTournament()}catch(err){setMessage(err.message)}}}>Bắt đầu</button>}
            {st==="inprogress"&&<span style={{fontSize:11,color:"#92400e"}}>Đang đua - chờ trọng tài nộp kết quả</span>}
            {st==="awaitingresult"&&<span style={{fontSize:11,color:"#92400e"}}>Chờ trọng tài nộp lại kết quả</span>}
            {st==="resultpendingapproval"&&<><button style={{padding:"6px 14px",fontSize:12,borderRadius:6,border:"none",background:"#1a7d1a",color:"#fff",cursor:"pointer",fontWeight:600}} onClick={async(e)=>{e.stopPropagation();if(!window.confirm("Duyệt kết quả này? Sau khi duyệt bạn có thể kết thúc cuộc đua."))return;try{await request(`/api/admin/races/${id}/approve-result`,{method:"POST"});setMessage("Đã duyệt kết quả!");viewTournament()}catch(err){setMessage(err.message)}}}>Duyệt KQ</button><button style={{padding:"6px 14px",fontSize:12,borderRadius:6,border:"none",background:"#c41e1e",color:"#fff",cursor:"pointer",fontWeight:600}} onClick={async(e)=>{e.stopPropagation();const reason=window.prompt("Lý do từ chối:");if(!reason)return;try{await request(`/api/admin/races/${id}/reject-result`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({reason})});setMessage("Đã từ chối kết quả.");viewTournament()}catch(err){setMessage(err.message)}}}>Từ chối</button></>}
            {st==="resultapproved"&&<button style={{padding:"6px 14px",fontSize:12,borderRadius:6,border:"none",background:"#1a7d1a",color:"#fff",cursor:"pointer",fontWeight:600}} onClick={async(e)=>{e.stopPropagation();if(!window.confirm("Kết thúc cuộc đua? Dự đoán sẽ được thanh toán theo tỉ lệ cược."))return;try{await request(`/api/races/management/${id}/end`,{method:"POST"});setMessage("Đã kết thúc và thanh toán dự đoán!");viewTournament()}catch(err){setMessage(err.message)}}}>Kết thúc</button>}
            ...
            <button className="admin-danger" style={{padding:"4px 10px",fontSize:11}} onClick={async(e)=>{e.stopPropagation();if(!window.confirm("Xóa?"))return;try{await request(`/api/races/management/${id}`,{method:"DELETE"});setMessage("Đã xóa.");viewTournament()}catch(err){setMessage(err.message)}}}>Xóa</button>
            <span style={{fontSize:11,color:"#657086"}}>{exp?"▲":"▼"}</span>
          </div>
        </div>
        {exp&&<div style={{padding:"0 16px 16px",borderTop:"1px solid rgba(143,100,32,0.1)"}}>
          <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(120px,1fr))",gap:10,padding:"12px 0"}}>
            <div><span style={{fontSize:10,color:"#657086",display:"block"}}>Khoảng cách</span><strong style={{fontSize:14,color:"#172033"}}>{(race.distance??race.Distance??0)}m</strong></div>
            <div><span style={{fontSize:10,color:"#657086",display:"block"}}>Ngựa</span><strong style={{fontSize:14,color:"#172033"}}>{det.entries?.length||(race.entriesCount??race.EntriesCount??0)}/{race.maxParticipants??race.MaxParticipants??12}</strong></div>
            <div><span style={{fontSize:10,color:"#657086",display:"block"}}>Bắt đầu</span><strong style={{fontSize:14,color:"#172033"}}>{fmtDate2(race.scheduledAt??race.ScheduledAt)}</strong></div>
          </div>
          {det.entries?.length>0 && <div style={{marginTop:8}}>
            <h4 style={{fontSize:12,margin:"0 0 6px",color:"#172033"}}>Tỷ lệ cược (admin có thể chỉnh)</h4>
