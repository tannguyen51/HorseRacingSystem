...
async function pollExport(client, designId, exportJob, maxAttempts = 30) {
    let job = exportJob;
...
async function main() {
    ...
    if (!auth.isConfigured()) {
        console.error('ERROR: CANVA_CLIENT_ID and CANVA_CLIENT_SECRET must be set as environment variables.\n' +
    ...
    const server = new McpServer({ name: 'canva-mcp-server', version: '1.0.0' }, {
        capabilities: { tools: {} },
        instructions: 'Canva MCP Server — create and manage Canva presentations, documents, and designs.\n' +
    ...
    // --- Register tools ---
    // 1. Auth status
    server.registerTool('canva_auth_status', {
        title: 'Canva Auth Status',
        description: 'Check Canva authentication status. If not authenticated, returns the OAuth authorization URL to open in a browser.',
    ...
    server.registerTool('canva_authenticate', {
        title: 'Authenticate with Canva',
        description: 'Complete OAuth authentication by exchanging the authorization code for an access token. Get the code from the redirect URL after authorizing in the browser.',
    ...
    // 3. Create design
    server.registerTool('canva_create_design', {
        title: 'Create Canva Design',
        description: 'Create a new Canva design (presentation, document, whiteboard, etc.). Returns the design ID for subsequent operations.',
        ...
        try {
            ...
            const message = error instanceof Error ? error.message : String(error);
            const detail = axios.isAxiosError(error) && error.response?.data
                ? formatResult(error.response.data)
                : '';
            return {
                content: [
                    { type: 'text', text: `❌ Failed: ${message}${detail ? '\n' + detail : ''}` },
                ],
                isError: true,
            };
        }
    ...
    // 4. Add page
    server.registerTool('canva_add_page', {
    ...
    // 5. Add text
    server.registerTool('canva_add_text', {
        title: 'Add Text Element',
        description: 'Add a text element to a page/slide. Position values are fractions (0.0-1.0) relative to page dimensions.',
    ...
    // 6. Add shape
    server.registerTool('canva_add_shape', {
        title: 'Add Shape Element',
        description: 'Add a shape (rectangle, circle, line, triangle) to a page/slide.',
    ...
    // 7. Upload asset
    server.registerTool('canva_upload_asset', {
        title: 'Upload Image Asset',
        description: 'Upload an image (from a URL) to Canva for use in designs. Returns an asset_id for use with canva_add_image.',
    ...
    // 8. Add image
    server.registerTool('canva_add_image', {
    ...
    // 9. Export design
    server.registerTool('canva_export_design', {
        title: 'Export Design',
        description: 'Export a Canva design to PDF, PNG, or JPG. Waits for the export to complete and returns download URLs.',
    ...
    // 10. Get design
    server.registerTool('canva_get_design', {
        title: 'Get Design Details',
        description: 'Get full details of a Canva design including all pages and their elements.',
        inputSchema: GetDesignInputSchema,
    }, async ({ design_id }) => {
        try {
            const design = await client.getDesign(design_id);
            return {
                content: [{ type: 'text', text: formatResult(design) }],
            };
        }
        catch (error) {
            const message = error instanceof Error ? error.message : String(error);
    ...
    // 11. List designs
    server.registerTool('canva_list_designs', {
        title: 'List Designs',
